...
public class Display {
	...
	public void shuffleList() {
		createList();
		for(int i = 0; i < 600; i++) {	//SHUFFLE RUNS 500 TIMES
			for(int j = 0; j < len; j++) {	//ACCESS EACH ELEMENT OF THE LIST
				int rand = r.nextInt(len);	//PICK A RANDOM NUM FROM 0-LEN
				int temp = list[j];			//SETS TEMP INT TO CURRENT ELEMENT
				list[j] = list[rand];		//SWAPS THE CURRENT INDEX WITH RANDOM INDEX
	...
	public void initialize() {
		...
		jf.setVisible(true);
		jf.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//Method for closing the window
		jf.setResizable(false);//This function will restrict the user to enlarge the screen
		jf.setLocationRelativeTo(null);//This function center the jFrame on the screen
		...
		credit.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(jf, "	                  Sorting Visualiser\n"
												+ "             Copyright (c) 2020-2021\n"
												+ "                       Prithwish Das\n"
												+ "         Build Date:  January 17, 2021   ", "Credit", JOptionPane.PLAIN_MESSAGE, new ImageIcon(""));
			}
		});
...
//				i = 0;
	...
	/*The purpose of extending a JPanel is to override the paintComponent method. 
	 The JPanel is invisible until we override paintComponent. If we need to draw something then
	 we need to override the method paintComponent method.
	 
	 super.paintComponent(g) --> The JPanel is our canvas and the graphics object is our paintbrush
	 and super.paintComponent(g) is our eraser. The super.paintComponent(g) invokes the paintComponent
	 method from the super class of JPanel(the JCotaner class) to erase whatever is currently
	  drawn on the panel*/
		...
		public void paintComponent(Graphics g) {
			...
			for(int i = 0; i < len; i++) {	//RUNS TROUGH EACH ELEMENT OF THE LIST
				...
				if(type == 0) {		//BAR GRAPH TYPE
					...
					/*This is to fill the rectangular bars*/
					g.fillRect(i*width, SIZE-HEIGHT, width, HEIGHT);
					g.setColor(Color.black);//This is for the border of the bars 
					g.drawRect(i*width, SIZE-HEIGHT, width, HEIGHT);//This to draw the rectangular bars with the black borders
				}
	...
	class SortingAlgorithms {
		...
		public void bubbleSort1() {
			for(int i=0;i<len-1;i++) {
				current=i;
				for(int j=0;j<len-i-1 && sorting;j++) {
					if(!sorting)
						break;
					if(list[j]>list[j+1]) {
						swap(j,j+1);
